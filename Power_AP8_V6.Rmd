---
title: 'Power Simulation: Metacognitive Effects of Attitudinal (In-)Congruence in
...
This is the power simulation for the study "Metacognitive Effects of Attitudinal (In-)Congruence in Social Media on Political Participation". It estimates power for three hypotheses referring to effects of attitudinal congruence on subjective knowledge, effects of attitudinal congruence on political participation, and the relationship between these two variables. The two remaining hypotheses regarding processing fluency and perceived controversiality are not addressed separately since past literature does not allow adequate estimation of effects. However, we believe the calculated sample size to be large enough to detect any effects of practical relevance for these hypotheses as well. 
<br>

### 1. Summary of Results

Based on the below analyses, we want to collect data of 1,400 participants. Regarding the group differences, this allows the detection of small standardized effect sizes of 0.14 with power of 1 - $\beta$ $\approx$ .85. Regarding the regression hypothesis, this sample size allows the detection of a small standardized effect of b = 0.08 with power of 1 - $\beta$ $\approx$ 0.90. For a more detailed explanation of why we chose these values, which effects we expect, as well as exact power estimates, please consult the remaining document. <br>

### 2. Calculation and Detailed Explanation 

Our estimates of the effect sizes are based on a paper of Wojcieszak et al. (2016), which we believe to be the published study most similar to our aim. See [section 3](#resultsW) for a summary of their effects. However, we assume our results will be somewhat larger than the results displayed in their paper for the following reasons: <br>
Our study implements a different media context and therefore a different operationalization of the independent variable attitudinal congruence. The Wojcieszak paper presented the participants news articles that were written in a balanced, pro- or counter-attitudinal way. We present the participants social media news posts with opinionated user generated comments. We believe this manipulation to evoke stronger perceptions of attitudinal (in)congruence. Further, they do not only entail the opinion of one author but of several commenters. <br><br>

Of course, we acknowledge that it would be best to achieve a sample size being able to detect the effects in the Wojcieszak paper as it is closest to our experiment. Due to limited ressources,this unfortunately is not possible, although we tried to choose a sample size that at least has reasonable power (>.60) also for these small effets. We further are still confident that our effects should be somewhat larger and that our sample size will suffice to detect any meaningful effects. 
To ensure full transparency and interpretability, we nonetheless report simulated power for the exact effects that Wojcieszak et al. (2016) observed in [section 2.2](#Wojcieszak) of this document. <br>

#### 2.1 Power Simulation 

The data is simulated so that there is a group difference in Subjective Knowledge of 0.2 units of measurement (1 - 7 scale, $sd_{SK}$ = 1.4, standardized = 0.14). Political Participation (PP) is simulated to partially depend on Subjective Knowledge (b = 0.09, standardized b = 0.084) but to also have a group difference of 0.24 units of measurement (1 - 7 scale, $sd_{PP}$ = 1.55) independent of Subjective Knowledge, resulting in an average group difference of 0.258 units of measurement (standardized = 0.17). <br>
For the relationship between SK and PP we use the value observed by Wojcieszak et al. (2016). For the group differences, we use slightly higher values (again, see [section 2.2](#Wojcieszak) for the simulations using the exact Wojcieszak results).
...
while(power_SK < 0.85){
  for(i in 1:nsim){
    ...
    SK1 <- rnorm(n, mean = 5.3, sd = 1.4) # data group 1 (congruent)
    ...
    data[data$group == 0, ]$PP <- data[data$group == 0, ]$PP + rnorm(n, 0.12, 0.1)
    data[data$group == 1, ]$PP <- data[data$group == 1, ]$PP - rnorm(n, 0.12, 0.1)
...
library(knitr)
final_result <- which(Results$GroupSize == 700)
kable(Results, align = rep("l", ncol(Results))) %>%
  row_spec(final_result, bold = T, background = "pink")  %>%
...
while(power_SK < 0.8 | power_PP < 0.8){
  for(i in 1:nsim){
